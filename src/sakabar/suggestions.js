...
export async function getFastSuggestions (searchText) {
  try {
    return searchText === ''
      ? []
      : [
...
export async function getSlowSuggestions (searchText) {
...
async function tabSuggestions (searchText) {
  ...
  console.log('tabs');
  return results.slice(0, MAX_RESULTS).map(({ item: { title, url }, score }) => ({
    type: 'tab',
    score,
    title,
    url
